...
int Authenticate_User(int client_Id, Server *server_Cls, int pos)
{
    ...
    std::cout << " --> " << pos << " " << server_Cls->poll_array.size() <<  std::endl ;
    memset(Recv_Buffer, 0, sizeof(Recv_Buffer));
    server_Cls->Size_Read = recv(server_Cls->start->fd, Recv_Buffer[3], sizeof(Recv_Buffer) , 0);
    ...
    Check_Commands(server_Cls, Recv_Buffer[3], server_Cls->start->fd);
    return (0);
};
